  ...
  <div id="codeListContainer" class="code-container full-height">
    <div class="full-height" style="padding: 10px">
      <vxe-grid
        ...
        <template #table-buttons>
          <a-form style="padding-left: 5px" layout="inline">
            <a-form-item>
              <a-input
                v-model:value="searchModel.tableName"
                :size="formSizeConfig"
            ...
            <a-form-item>
              <a-select
                ...
                <a-select-option v-for="(value, key) in tableTypeList" :key="key" :value="key">
                  {{ value }}
                </a-select-option>
              </a-select>
            </a-form-item>
            <a-form-item>
              <a-button :size="buttonSizeConfig" type="primary" @click="doLoadData">
            ...
            <template #overlay>
              <a-menu @click="({ key }) => handleActions(row.id, key)">
                ...
                <a-menu-item key="create">
                  {{ $t('generator.views.code.button.createCode') }}
                </a-menu-item>
              </a-menu>
            </template>
          </a-dropdown>
      ...
      <a-modal
        ...
        :title="$t('generator.views.code.title.design')"
        width="100%"
        ...
        <a-spin :spinning="configDataLoading">
              ...
              <a-list-item>
                ...
                <a-col :span="span">
                  <a-form-item name="tableName" :label="$t('generator.views.code.table.tableName')">
                    <a-input v-model:value="formModel.tableName" :size="formSize" />
                  </a-form-item>
                </a-col>
                <a-col :span="span">
                  <a-form-item
                    ...
                    <a-input v-model:value="formModel.configName" :size="formSize" />
                  </a-form-item>
                </a-col>
                <a-col :span="span">
              ...
              <a-list-item>
                <a-col :span="span">
                  <a-form-item :label="$t('generator.views.code.title.leftButton')">
                    <a-select
                      ...
                      style="width: 100%"
                      mode="multiple">
                      <a-select-option v-for="item in buttonList" :key="item.key" :value="item.key">
                        {{ item.value }}
                      </a-select-option>
                    </a-select>
                  </a-form-item>
                ...
                <a-col :span="span">
                  <a-form-item :label="$t('generator.views.code.title.rowButtonType.title')">
                    <a-select
                      v-model:value="formModel.rowButtonType"
                      :size="formSize"
                      style="width: 100%">
                      <a-select-option
                  ...
                  <a-form-item
                    ...
                    <a-select v-model:value="formModel.formColNum" :size="formSize">
                      <a-select-option :value="1">
                        {{ $t('generator.views.code.title.colNum.one') }}
                      </a-select-option>
                      <a-select-option :value="2">
                        {{ $t('generator.views.code.title.colNum.two') }}
                      </a-select-option>
                      <a-select-option :value="3">
                        {{ $t('generator.views.code.title.colNum.three') }}
                      </a-select-option>
                      <a-select-option :value="4">
                        {{ $t('generator.views.code.title.colNum.four') }}
                      </a-select-option>
                    </a-select>
                  </a-form-item>
                </a-col>
                ...
                <a-col :span="span">
                  <a-form-item :label="$t('generator.views.code.title.relateTable')">
                    ...
                    <PlusOutlined
                      ...
                      @click="handleShowChoseAddendumModal" />
                  </a-form-item>
                </a-col>
                <a-col :span="span">
                  <a-form-item>
                    <a-button type="primary" :size="buttonSizeConfig" @click="handleSyncTableData">
          ...
          <a-tabs :animated="true">
            <a-tab-pane
              key="1"
              class="full-height"
              :tab="$t('generator.views.code.title.dbMessage')">
              <TableFieldTable
                class="full-height"
...
export default defineComponent({
  ...
  setup() {
    ...
    // 保存配置操作
    const vueSaveConfig = vueSaveConfigSupport(
      t,
      isSync,
      loadTableVue.doLoadData,
      addEditModalVisible,
      dbData,
      editConfigData,
      choseTableList,
    ...
    // 编辑配置VUE对象
    const { configDataLoading, handleShowEdit } = vueConfigEditSupport(
      addEditModalVisible,
      vueSaveConfig.formModel,
      vueLoadDbData.loadDbData,
      editConfigData,
      choseTableList,
    )
    // 操作vue
    ...
    return {
      ...
      formRules: {
        connectionId: [
          {
            required: true,
            message: this.$t('generator.views.code.validate.connectionName'),
            trigger: 'change',
          },
        ],
        tableName: [
          {
            required: true,
            message: this.$t('generator.views.code.validate.tableName'),
            trigger: 'blur',
          },
        ],
        configName: [
          {
            required: true,
            message: this.$t('generator.views.code.validate.configName'),
            trigger: 'blur',
          },
        ],
        i18nPrefix: [
          {
            required: true,
            message: this.$t('generator.views.code.validate.i18nPrefix'),
            trigger: 'blur',
          },
        ],
      },
