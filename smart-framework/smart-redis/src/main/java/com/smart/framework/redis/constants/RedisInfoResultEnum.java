package com.smart.framework.redis.constants;
...
public enum RedisInfoResultEnum {
    ...
    TOTAL_NET_INPUT_BYTES(RedisInfoParameterEnum.STATS, "total_net_input_bytes", "redis 网络入口流量字节数"),
    TOTAL_NET_OUTPUT_BYTES(RedisInfoParameterEnum.STATS, "total_net_output_bytes", "redis 网络出口流量字节数"),
    INSTANTANEOUS_INPUT_KBPS(RedisInfoParameterEnum.STATS, "instantaneous_input_kbps", "redis 网络入口 kps"),
    INSTANTANEOUS_OUTPUT_KBPS(RedisInfoParameterEnum.STATS, "instantaneous_output_kbps", "redis 网络出口 kps"),
     ...
     * replication
     */
    ROLE(RedisInfoParameterEnum.REPLICATION, "role", "如果当前服务器没有在复制任何其他服务器，那么这个域的值就是 master ；否则的话，这个域的值就是 slave 。注意，在创建复制链的时候，一个从服务器也可能是另一个服务器的主服务器。"),
