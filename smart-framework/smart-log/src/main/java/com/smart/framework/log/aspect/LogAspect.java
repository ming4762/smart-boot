package com.smart.framework.log.aspect;
...
public final class LogAspect {
    ...
    private static final Set<Class<?>> EXCLUDE_CLASS = Set.of(ServletRequest.class, InputStreamResource.class);

    /**
     * 保存日志的编码集合
     */
    private final List<Integer> codeList;
    private final List<LogHandler> logHandlerList;
    private final boolean console;

    public LogAspect(Boolean console, String codes, List<LogHandler> logHandlerList) {
        this.logHandlerList = logHandlerList;
        this.console = Boolean.TRUE.equals(console);
     ...
     * 切面
     */
    @Pointcut("@annotation(com.smart.framework.commons.core.log.Log)")
    ...
    public void before(JoinPoint point) {
        if (this.console) {
            ...
            // 打印调用 com.smart.framework.tool.code.controller 的全路径以及执行方法
    ...
    @After("logPointCut()")
    public void after() {
        if (BooleanUtils.isTrue(this.console)) {
            log.info("=========================================== End ===========================================");
        }
    }
    ...
    private void doSaveLog(ProceedingJoinPoint point, @Nullable Object result, Signature signature, Log logAnnotation, long time, int code, String errorMessage) {
        ...
        this.logHandlerList.forEach(item -> {
            try {
                item.save(sysLog, point, logAnnotation, time, code, result, errorMessage);
            } catch (Exception e) {
