...
public class SqlServerDatabaseExecutor extends AbstractDefaultDatabaseExecutor implements DatabaseExecutor {
     ...
     * 查询列备注SQL
     */
    private static final String COLUMN_COMMENTS_SQL = "select a.NAME COLUMN_NAME, c.name TABLE_NAME,cast(isnull(e.[value],'') as nvarchar(100)) as REMARK from sys.columns a inner join sys.objects c on a.object_id=c.object_id and c.com.smart.framework.tool.code.type='u' left join sys.extended_properties e on e.major_id=c.object_id and e.minor_id=a.column_id and e.class=1 where c.name in %in";
