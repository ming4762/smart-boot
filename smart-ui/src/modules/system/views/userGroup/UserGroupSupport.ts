...
export const vueDelete = (gridRef: Ref, loadData: any) => {
  ...
  const handleDelete = () => {
    ...
    Modal.confirm({
      ...
      content: '确定要删除吗？',
      onOk: async () => {
        await ApiService.postAjax('sys/userGroup/batchDeleteById', selectRows.map((item: any) => item.groupId))
        loadData()
      }
    })
  }
...
export const vueSetUser = () => {
  ...
  const handleShowSetUser = async (row: any) => {
    ...
    try {
      ...
      // 加载用户关联数据
      const result: Array<number> = await ApiService.postAjax('sys/userGroup/listUserIdById', selectRow.groupId)
