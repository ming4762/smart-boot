  ...
  <div class="full-height">
    <vxe-grid
      ...
      <template #table-tableName="{ row }">
        <a-input v-model:value="row.tableName" :disabled="!row.useTableSearch" :size="formSizeConfig" />
      </template>
      <template #table-keyColumnName="{ row }">
        <a-input v-model:value="row.keyColumnName" :disabled="!row.useTableSearch" :size="formSizeConfig" />
      </template>
      <template #table-valueColumnName="{ row }">
        <a-input v-model:value="row.valueColumnName" :disabled="!row.useTableSearch" :size="formSizeConfig" />
      </template>
      <template #table-tableWhere="{ row }">
        <a-input v-model:value="row.tableWhere" :disabled="!row.useTableSearch" :size="formSizeConfig" />
      </template>
      ...
      <template #table-rules="{ row }">
        <a-button :size="tableButtonSizeConfig" @click="() => handleShowSetRules(row)">设置规则</a-button>
      </template>
    ...
    <!--  验证规则  -->
    <FormRuleSelect v-model:visible="setRuleModalVisible" :rules="currentRow.ruleList === null ? [] : currentRow.ruleList" @save="handleSave" />
  </div>
</template>

<script lang="ts">
import { defineComponent, PropType, ref, onMounted, watch, toRefs, Ref } from 'vue'
