  ...
  <div class="full-height">
    <vxe-grid
      ...
      <template #table-readonly="{ row }">
        <a-checkbox v-model:checked="row.readonly" :disabled="!row.visible" :size="formSizeConfig" />
      </template>
      ...
      <template #table-searchSymbol="{ row }">
        <a-select v-model:value="row.searchSymbol" :disabled="!row.visible" :size="formSizeConfig" style="width: 100px">
          <a-select-option
      ...
      <template #table-useTableSearch="{ row }">
        <a-checkbox v-model:checked="row.useTableSearch" :disabled="!row.visible" :size="formSizeConfig" />
      </template>
      <template #table-tableName="{ row }">
        <a-input v-model:value="row.tableName" :disabled="!(row.useTableSearch && row.visible)" :size="formSizeConfig" />
      </template>
      <template #table-keyColumnName="{ row }">
        <a-input v-model:value="row.keyColumnName" :disabled="!(row.useTableSearch && row.visible)" :size="formSizeConfig" />
      </template>
      <template #table-valueColumnName="{ row }">
        <a-input v-model:value="row.valueColumnName" :disabled="!(row.useTableSearch && row.visible)" :size="formSizeConfig" />
      </template>
      <template #table-tableWhere="{ row }">
        <a-input v-model:value="row.tableWhere" :disabled="!(row.useTableSearch && row.visible)" :size="formSizeConfig" />
      </template>
      <template #table-controlType="{ row }">
        <a-select v-model:value="row.controlType" :disabled="!row.visible" style="width: 100px" :size="formSizeConfig">
          <a-select-option
        ...
        <a-tooltip v-if="row.controlType === 'selectTable'" placement="top">
          ...
          <PlusOutlined :style="{ cursor: 'pointer', 'margin-left': '5px' }" @click="() => handleShowChoseSelectTable(row)" />
        </a-tooltip>
      </template>
      <template #table-rules="{ row }">
        <a-select v-model:value="row.rules" :disabled="!row.visible" mode="multiple" :size="formSizeConfig" style="width: 160px">
          <a-select-option
    ...
    <PageAddendumTableChose v-model:visible="choseAddendumModalVisible" :select-table-list="currentRow.selectTableList == null ? [] : currentRow.selectTableList" :multiple="false" @chose="handleChoseTable" />
  </div>
</template>
...
import { controlList, ruleList, searchSymbolList, vueTableHeaderCheckboxSupport, vueChoseSelectTableSupport } from './PageSettingSupport'
...
export default defineComponent({
  ...
  setup (props) {
    ...
    return {
      ...
      ...vueSupportVxeSortable(tableRef, '.table-drop', data),
      ...
      headerReadonlyCheckboxChecked: vueTableHeaderCheckboxSupport(data, 'readonly', false).checked,
