<template>
  <div :class="[prefixCls, getAlign]" @click="onCellClick">
    <template v-for="(action, index) in getActions" :key="`${index}-${action.label}`">
      ...
      <template v-else>
        <el-tooltip v-if="!action.hasAuth" :title="$t('common.message.noPermission')" color="red">
          <!--   无权限  -->
        ...
        <el-tooltip v-else-if="action.tooltip" v-bind="getTooltip(action.tooltip)">
...
export default defineComponent({
  name: 'TableAction',
  components: { Icon, PopConfirmVxeButton, Dropdown },
  props: {
    actions: {
  ...
  setup(props) {
    ...
    function getTooltip(data: string) {
      return {
        getPopupContainer: () => unref((table as any)?.wrapRef.value) ?? document.body,
        placement: 'bottom',
